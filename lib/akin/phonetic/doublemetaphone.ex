defmodule Akin.Phonetic.DoubleMetaphone do
  ...
  alias Word
  alias Akin.Phonetic.DoubleMetaphone
  ...
  """
  def init(input) do
    %DoubleMetaphone{word: Word.init(input)}
  end
  ...
  """
  def check_word_start(%DoubleMetaphone{position: position} = metaphone) do
  ...
  """
  def process_initial_vowels(%DoubleMetaphone{} = metaphone, position) do
  ...
  def process(%DoubleMetaphone{position: position, word: %Word{start_index: start_index}} = metaphone, character)
  when character == "C" do
    if (position > (start_index + 1)
    and letter_at_position(metaphone, (position - 2)) not in @vowels
    and letter_at_position(metaphone, (position - 1), (position + 2)) == "ACH"
    and letter_at_position(metaphone, (position + 2)) not in ["I"]
    and (letter_at_position(metaphone, (position + 2)) not in ["E"]
    or letter_at_position(metaphone, (position - 2), (position + 4)) in [
      "BACHER", "MACHER"
    ])) do
      %{metaphone | next: {"K", 2}}
    else
      if position == start_index
      and letter_at_position(metaphone, start_index, (start_index + 6)) == "CAESAR" do
        %{metaphone | next: {"S", 2}}
      else
        if letter_at_position(metaphone, position, (position + 4)) == "CHIA" do
          ...
          if letter_at_position(metaphone, position, (position + 2)) == "CH" do
            if (position > start_index
            and letter_at_position(metaphone, position, (position + 4)) == "CHAE") do
              %{metaphone | next: {"K", "X", 2}}
            else
              if (position == start_index
              and letter_at_position(metaphone, (position + 1), (position + 6)) in ["HARAC", "HARIS"]
              or letter_at_position(metaphone, (position + 1), (position + 4)) in [
                "HOR", "HYM", "HIA", "HEM"
              ])
              and letter_at_position(metaphone, start_index, (start_index + 5)) != "CHORE" do
                %{metaphone | next: {"K", 2}}
              else
                if (letter_at_position(metaphone, start_index, (start_index + 4)) in ["VAN", "VON"]
                or letter_at_position(metaphone, start_index, start_index + 3) == "SCH"
                or letter_at_position(metaphone, (position - 2), (position + 4)) in [
                  "ORCHES", "ARCHIT", "ORCHID"
                ]
                or letter_at_position(metaphone, (position + 2))  in ["T", "S"]
                or ((letter_at_position(metaphone, (position - 1)) in ["A", "O", "U", "E"]
                or position == start_index)
                and letter_at_position(metaphone, (position + 2)) in [
                  "L", "R", "N", "M", "B", "H", "F", "V", "W", " "
                ])) do
                  %{metaphone | next: {"K", 2}}
                else
            ...
            if letter_at_position(metaphone, position, (position + 2)) == "CZ"
            and letter_at_position(metaphone, (position - 2), (position + 2)) != "WICZ" do
              %{metaphone | next: {"S", "X", 2}}
            else
              if letter_at_position(metaphone, (position + 1), (position + 4)) == "CIA" do
                %{metaphone | next: {"X", 3}}
              else
                if letter_at_position(metaphone, position, (position + 2)) == "CC"
                and not (position == (start_index + 1)
                and letter_at_position(metaphone, start_index) == "M") do
                  if letter_at_position(metaphone, (position + 2)) in ["I", "E", "H"]
                  and letter_at_position(metaphone, (position + 2), (position + 4)) != "HU" do
                    if ((position == (start_index + 1)
                    and letter_at_position(metaphone, start_index) == "A")
                    or letter_at_position(metaphone, (position - 1), (position + 4)) in [
                      "UCCEE", "UCCES"
                    ]) do
                      %{metaphone | next: {"KS", 3}}
                  ...
                  if letter_at_position(metaphone, position, (position + 2)) in ["CK", "CG", "CQ"] do
                    ...
                    if letter_at_position(metaphone, position, (position + 2)) in ["CI", "CE", "CY"] do
                      if letter_at_position(metaphone, position, (position + 3)) in ["CIO", "CIE", "CIA"] do
                        %{metaphone | next: {"S", "X", 2}}
                      else
                        %{metaphone | next: {"S", 2}}
                      end
                    else
                      if letter_at_position(metaphone, (position + 1), (position + 3)) in [" C", " Q", " G"] do
                        %{metaphone | next: {"K", 3}}
                      else
                        if letter_at_position(metaphone, (position + 1)) in ["C", "K", "Q"]
                        and letter_at_position(metaphone, (position + 1), (position + 3)) not in ["CE", "CI"] do
                          %{metaphone | next: {"K", 2}}
                        else
                          %{metaphone | next: {"K", 1}}
                        end
  ...
  def process(%DoubleMetaphone{position: position} = metaphone, character) when character == "D" do
  ...
  def process(%DoubleMetaphone{position: position} = metaphone, character) when character == "F" do
  ...
  when character == "G" do
    if letter_at_position(metaphone, (position + 1)) == "H" do
      if position > start_index
      and letter_at_position(metaphone, (position - 1)) not in @vowels do
        %{metaphone | next: {"K", 2}}
      else
        if position < (start_index + 3) do
          ...
          if ((position > (start_index + 1)
          and letter_at_position(metaphone, (position - 2)) in ["B", "H", "D"])
          or (position > (start_index + 2)
          and letter_at_position(metaphone, (position - 3)) in ["B", "H", "D"])
          or (position > start_index + 3
          and letter_at_position(metaphone, (position - 4)) in ["B", "H"])) do
            %{metaphone | next: {nil, 2}}
          else
            if position > start_index + 2
            and letter_at_position(metaphone, (position - 1)) == "U"
            and letter_at_position(metaphone, (position - 3)) in ["C", "G", "L", "R", "T"] do
              %{metaphone | next: {"F", 2}}
            else
              if position > start_index
              and letter_at_position(metaphone, (position - 1)) != "I" do
                %{metaphone | next: {"K", 2}}
              else
                %{metaphone | next: {nil, 2}}
              end
      ...
      if letter_at_position(metaphone, (position + 1)) == "N" do
        if position == start_index + 1
        and letter_at_position(metaphone, start_index) in @vowels
        and not Word.is_slavo_germanic?(metaphone.word) do
          %{metaphone | next: {"KN", "N", 2}}
        else
          if letter_at_position(metaphone, (position + 2), (position + 4)) != "EY"
          and letter_at_position(metaphone, (position + 1)) != "Y"
          and not Word.is_slavo_germanic?(metaphone.word) do
        ...
        if letter_at_position(metaphone, (position + 1), (position + 3)) == "LI"
        and not Word.is_slavo_germanic?(metaphone.word) do
          %{metaphone | next: {"KL", "L", 2}}
        else
          if (position == start_index
          and (letter_at_position(metaphone, (position + 1)) == "Y"
          or letter_at_position(metaphone, (position + 1), (position + 3)) in [
            "ES", "EP", "EB", "EL", "EY", "IB", "IL", "IN", "IE", "EI", "ER"
          ])) do
            %{metaphone | next: {"K", "J", 2}}
          else
            if ((letter_at_position(metaphone, (position + 1), (position + 3)) == "ER"
            or letter_at_position(metaphone, (position + 1)) == "Y")
            and letter_at_position(metaphone, start_index, (start_index + 6)) not in [
              "DANGER", "RANGER", "MANGER"
            ]
            and letter_at_position(metaphone, (position - 1)) not in ["E", "I"]
            and letter_at_position(metaphone, (position - 1), (position + 2)) not in ["RGY", "OGY"]) do
              %{metaphone | next: {"K", "J", 2}}
            else
              if letter_at_position(metaphone, (position + 1)) in ["E", "I", "Y"]
  ...
  def process(%DoubleMetaphone{position: position} = metaphone, character) when character == "H" do
    if (position == metaphone.word.start_index)
       or (letter_at_position(metaphone, (position - 1)) in @vowels
       and letter_at_position(metaphone, (position + 1)) in @vowels) do
      %{metaphone | next: {"H", 2}}
    else
      %{metaphone | next: {nil, 1}}
    end
  ...
  def process(%DoubleMetaphone{position: position, word: %Word{start_index: start_index}} = metaphone, character)
  when character == "J" do
    metaphone = if letter_at_position(metaphone, position, (position + 4)) == "JOSE"
      or letter_at_position(metaphone, start_index, (start_index + 4)) == "SAN " do
          if (position == start_index and letter_at_position(metaphone, (position + 4)) == " ")
          or letter_at_position(metaphone, start_index, (start_index + 4)) == "SAN " do
        ...
        if position == start_index
        and letter_at_position(metaphone, position, (position + 4)) != "JOSE" do
          %{metaphone | next: {"J", "A"}}
        else
          if letter_at_position(metaphone, (position - 1)) in @vowels
          and not Word.is_slavo_germanic?(metaphone.word)
          and letter_at_position(metaphone, (position + 1)) in ["A", "O"] do
            %{metaphone | next: {"J", "H"}}
          else
            if position == metaphone.word.end_index do
              %{metaphone | next: {"J", " "}}
            else
              if letter_at_position(metaphone, (position + 1)) not in ["L", "T", "K", "S", "N", "M", "B", "Z"]
              and letter_at_position(metaphone, (position - 1)) not in ["S", "K", "L"] do
  ...
  def process(%DoubleMetaphone{} = metaphone, character) when character == "K" do
  ...
  def process(%DoubleMetaphone{position: position, word: %Word{end_index: end_index}} = metaphone, character)
  when character == "L" do
    if letter_at_position(metaphone, (position + 1)) == "L" do
      if ((position == (end_index - 2)
      and letter_at_position(metaphone, (position - 1), (position + 3)) in ["ILLO", "ILLA", "ALLE"])
      or ((letter_at_position(metaphone, (end_index - 1), (end_index + 1)) in ["AS", "OS"]
      or letter_at_position(metaphone, end_index) in ["A", "O"])
      and letter_at_position(metaphone, (position - 1), (position + 3))  == "ALLE")) do
        %{metaphone | next: {"L", "", 2}}
      else
        %{metaphone | next: {"L", 2}}
      end
  ...
  def process(%DoubleMetaphone{position: position, word: %Word{end_index: end_index}} = metaphone, character)
  when character == "M" do
    if (letter_at_position(metaphone, (position + 1), (position + 4)) == "UMB"
    and ((position + 1) == end_index
      or letter_at_position(metaphone, (position + 2), (position + 4)) == "ER"))
    or letter_at_position(metaphone, (position + 1)) == "M" do
      %{metaphone | next: {"M", 2}}
    else
      %{metaphone | next: {"M", 1}}
    end
  end
  def process(%DoubleMetaphone{position: position} = metaphone, character) when character == "N" do
    if letter_at_position(metaphone, (position + 1)) == "N" do
      %{metaphone | next: {"N", 2}}
    else
      %{metaphone | next: {"N", 1}}
  ...
  def process(%DoubleMetaphone{position: position} = metaphone, character) when character == "P" do
  ...
  def process(%DoubleMetaphone{position: position} = metaphone, character) when character == "Q" do
  ...
  def process(%DoubleMetaphone{position: position, word: %Word{end_index: end_index}} = metaphone, character)
  when character == "R" do
    metaphone = if position == end_index
      and not Word.is_slavo_germanic?(metaphone.word)
      and letter_at_position(metaphone, (position - 2), position) == "IE"
      and letter_at_position(metaphone, (position - 4), (position - 2)) not in ["ME", "MA"] do
        %{metaphone | next: {"", "R"}}
      else
  ...
  def process(%DoubleMetaphone{position: position, word: %Word{start_index: start_index, end_index: end_index}
  } = metaphone, character) when character == "S" do
    if letter_at_position(metaphone, (position - 1), (position + 2)) in ["ISL", "YSL"] do
      %{metaphone | next: {nil, 1}}
    else
      if position == start_index
      and letter_at_position(metaphone, start_index, (start_index + 5)) == "SUGAR" do
        ...
        if letter_at_position(metaphone, position, (position + 2)) == "SH" do
          ...
          if letter_at_position(metaphone, position, (position + 3)) in ["SIO", "SIA"]
          or letter_at_position(metaphone, position, (position + 4)) == "SIAN" do
            ...
            if ((position == start_index
            and letter_at_position(metaphone, (position + 1)) in ["M", "N", "L", "W"])
            or letter_at_position(metaphone, (position + 1)) == "Z") do
              ...
              if letter_at_position(metaphone, position, (position + 2)) == "SC" do
                if letter_at_position(metaphone, (position + 2)) == "H" do
                  if letter_at_position(metaphone, (position + 3), (position + 5)) in [
                    ...
                    if position == start_index
                    and letter_at_position(metaphone, (start_index + 3)) not in @vowels
                    and letter_at_position(metaphone, (start_index + 3)) != "W" do
                      %{metaphone | next: {"X", "S", 3}}
                    else
                      %{metaphone | next: {"X", 3}}
                    end
                ...
                and letter_at_position(metaphone, (position - 2), position) in ["AI", "OI"] do
                  %{metaphone | next: {"", "S", 1}}
                else
  ...
  when character == "T" do
    if letter_at_position(metaphone, position, (position + 4)) == "TION" do
      ...
      if letter_at_position(metaphone, position, (position + 3)) in ["TIA", "TCH"] do
        ...
        if letter_at_position(metaphone, position, (position + 2)) == "TH"
        or letter_at_position(metaphone, position, (position + 3)) == "TTH" do
          if letter_at_position(metaphone, (position + 2), (position + 4)) in ["OM", "AM"]
          or letter_at_position(metaphone, start_index, (start_index + 4)) in ["VON ", "VAN "]
          or letter_at_position(metaphone, start_index, (start_index + 3)) == "SCH" do
            %{metaphone | next: {"T", 2}}
          else
  ...
  def process(%DoubleMetaphone{position: position} = metaphone, character) when character == "V" do
  ...
  when character == "W" do
    if letter_at_position(metaphone, position, (position + 1)) == "WR" do
      ...
      if position == start_index
      and letter_at_position(metaphone, (position + 1)) in @vowels
      or letter_at_position(metaphone, position, (position + 2)) == "WH" do
        ...
        if ((position == metaphone.word.end_index
        and letter_at_position(metaphone, (position - 1)) in @vowels)
        or letter_at_position(metaphone, (position - 1), (position + 4)) in [
  ...
  def process(%DoubleMetaphone{position: position} = metaphone, character) when character == "X" do
    metaphone = %{metaphone | next: {nil, nil}}
    metaphone = if not (position == metaphone.word.end_index
      and letter_at_position(metaphone, (position - 3), position) in ["IAU", "EAU"]
      or letter_at_position(metaphone, (position - 2), position) in ["AU", "OU"]) do
  ...
  def process(%DoubleMetaphone{position: position} = metaphone, character) when character == "Z" do
    metaphone = if letter_at_position(metaphone, (position + 1)) == "H" do
        %{metaphone | next: {"J", nil}}
      else
        if letter_at_position(metaphone, (position + 1), (position + 3)) in [
          "ZO", "ZI", "ZA"
        ]
        or (Word.is_slavo_germanic?(metaphone.word)
        and position > metaphone.word.start_index
        and letter_at_position(metaphone, (position - 1)) != "T") do
          %{metaphone | next: {"S", "TS"}}
        else
          %{metaphone | next: {"S", nil}}
        end
      end
    if letter_at_position(metaphone, (position + 1))== "Z"
    or letter_at_position(metaphone, (position + 1))== "H" do
      %{metaphone | next: Tuple.append(metaphone.next, 2)}
    else
      %{metaphone | next: Tuple.append(metaphone.next, 1)}
    end
  ...
  defp build_phones(%DoubleMetaphone{next: {nil, next}, position: position} = metaphone) do
    %{metaphone | position: position + next}
  end
  defp build_phones(%DoubleMetaphone{next: {a, next}, position: position} = metaphone) do
    primary_phone = metaphone.primary_phone <> a
    secondary_phone = metaphone.secondary_phone <> a
    %{metaphone | position: position + next, primary_phone: primary_phone, secondary_phone: secondary_phone}
  end
  defp build_phones(%DoubleMetaphone{next: {nil, nil, next}, position: position} = metaphone) do
    %{metaphone | position: position + next}
  end
  defp build_phones(%DoubleMetaphone{next: {nil, b, next}, position: position} = metaphone) do
    secondary_phone = metaphone.secondary_phone <> b
    %{metaphone | position: position + next,  secondary_phone: secondary_phone}
  end
  defp build_phones(%DoubleMetaphone{next: {a, nil, next}, position: position} = metaphone) do
    primary_phone = metaphone.primary_phone <> a
    secondary_phone = metaphone.secondary_phone <> a
    %{metaphone | position: position + next,  primary_phone: primary_phone, secondary_phone: secondary_phone}
  end
  defp build_phones(%DoubleMetaphone{next: {a, b, next}, position: position} = metaphone) do
    primary_phone = metaphone.primary_phone <> a
    secondary_phone = metaphone.secondary_phone <> b
    %{metaphone | position: position + next, primary_phone: primary_phone, secondary_phone: secondary_phone}
  end
  defp build_phones(%DoubleMetaphone{position: position} = metaphone) do
    %{metaphone | position: position + 1}
  end

  defp return_phones({a, b}), do: {String.downcase(a), String.downcase(b)}

  defp parse_next(%DoubleMetaphone{} = metaphone) do
    position = metaphone.position
    end_index =  metaphone.word.end_index
    character = letter_at_position(metaphone, position)
    parse(metaphone, position, end_index, character)
  ...
  def letter_at_position(%DoubleMetaphone{} = metaphone, start_position) do
    String.slice(metaphone.word.buffer, start_position, 1)
  end
  def letter_at_position(%DoubleMetaphone{} = metaphone, start_position, close_position) do
